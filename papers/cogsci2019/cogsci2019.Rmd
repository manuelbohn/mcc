...
# data ex 3
dex3 <- read_csv(file="../../stats/data/adult_ex3.csv")

# data ex 4

dex4 <- read_csv(file="../../stats/data/adult_ex4.csv")

# loading model comparisons and predictions

pred_ex3 <- readRDS(file = "saves/model_predictions_ex3_1.rds")

pred_ex4 <- readRDS(file = "saves/model_predictions_ex3_variable_priors.rds")

bf_ex3 <- readRDS(file = "saves/ex3_bf.rds")

bf_ex4 <- readRDS(file = "saves/ex4_bf.rds")
...
lm_ex1 <- glmer(correct ~ condition +
              (1 |id), 
              data = dex12%>%filter(experiment == "adult_ex1"), 
...
Participants selected the less frequent object above chance in the test condition (t(`r ex1t%>%filter(condition == "test")%>%pull(df)`) = `r ex1t%>%filter(condition == "test")%>%pull(t_value)`, *p* `r ex1t%>%filter(condition == "test")%>%pull(p_value)`, see Fig. \ref{fig:plotexp12}) and did so more often compared to the control condition (generalized linear mixed model (GLMM\footnote{All models had maximal random effects structure conditional on model convergence.}):*$\beta$* = `r ex1_r%>%filter(term == "conditiontest")%>%pull(estimate)`, se = `r ex1_r%>%filter(term == "conditiontest")%>%pull(std.error)` *p* `r ex1_r%>%filter(term == "conditiontest")%>%pull(p.value)`). This result replicates earlier work [@frank2014inferring] and is consistent with the hypothesis that listeners expect speakers to communicate in an informative way.
...
p1 <- dex12 %>%
  mutate(condition = ifelse(condition == "control", "Control", "Test"),
...
p2 <- p1 %>%
 ...
 ggplot() +
  ...
  geom_pointrange(data = p2, aes(x = condition, y = mean, col = condition,ymin = ci_lower, ymax = ci_upper),size = .5)+
  geom_hline(yintercept = 0.5, lty=2)+
  labs(x="",y="Expected Choice")+
  facet_wrap( ~ experiment, labeller = as_labeller(c(`adult_ex2_preference`="Exp. 2 - Pref.", `adult_ex2_novelty`="Exp. 2 - Nov.", `adult_ex1`="Exp. 1 - Inf.")))+
...
```{r ex2-results-pref}
ex2tpref <- dex12 %>%
  filter(experiment == "adult_ex2_preference")%>%
  group_by(condition, id) %>%
  summarise(correct = mean(correct)) %>%
  summarise(correct = list(correct)) %>%
  group_by(condition) %>%
...
lm_ex2pref <- glmer(correct ~ condition +
              (1|id), 
              data = dex12%>%filter(experiment == "adult_ex2_preference"), 
              family = binomial,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
```{r ex2-results-nov}
ex2tnov <- dex12 %>%
  filter(experiment == "adult_ex2_novelty")%>%
  group_by(condition, id) %>%
  summarise(correct = mean(correct)) %>%
  summarise(correct = list(correct)) %>%
  group_by(condition) %>%
...
lm_ex2nov <- glmer(correct ~ condition +
              (condition|id), 
              data = dex12%>%filter(experiment == "adult_ex2_novelty"), 
              family = binomial,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
Participants selected the preferred object above chance when the same animal returned (t(`r ex2tpref%>%filter(condition == "test")%>%pull(df)`) = `r ex2tpref%>%filter(condition == "test")%>%pull(t_value)`, *p* `r ex2tpref%>%filter(condition == "test")%>%pull(p_value)`, see Fig. \ref{fig:plotexp12}) and did so more often compared to trials in which a different animal returned (GLMM: $\beta$ = `r ex2pref_r%>%filter(term == "conditiontest")%>%pull(estimate)`, se = `r ex2pref_r%>%filter(term == "conditiontest")%>%pull(std.error)`, *p* `r ex2pref_r%>%filter(term == "conditiontest")%>%pull(p.value)`). Thus, listeners inferred the referent of the utterance by considering previous interactions with the speaker. Interestingly, participants transferred preference to some extent from one animal to the other and selected the preferred object above chance when a different animal returned (t(`r ex2tpref%>%filter(condition == "control")%>%pull(df)`) = `r ex2tpref%>%filter(condition == "control")%>%pull(t_value)`, *p* `r ex2tpref%>%filter(condition == "control")%>%pull(p_value)`). In sum, this study shows that adults make comparable inferences to children [@saylor2009preschoolers].

The novel object was selected above chance when the same animal returned (t(`r ex2tnov%>%filter(condition == "test")%>%pull(df)`) = `r ex2tnov%>%filter(condition == "test")%>%pull(t_value)`, *p* `r ex2tnov%>%filter(condition == "test")%>%pull(p_value)`) but not when a different one appeared (t(`r ex2tnov%>%filter(condition == "control")%>%pull(df)`) = `r ex2tnov%>%filter(condition == "control")%>%pull(t_value)`, *p* `r ex2tnov%>%filter(condition == "control")%>%pull(p_value)`, see Fig. \ref{fig:plotexp12}). Furthermore, the two conditions differed in the expected direction ($\beta$ = `r ex2nov_r%>%filter(term == "conditiontest")%>%pull(estimate)`, se = `r ex2nov_r%>%filter(term == "conditiontest")%>%pull(std.error)`, *p* `r ex2nov_r%>%filter(term == "conditiontest")%>%pull(p.value)`). Thus, like children [@akhtar1996role], adults used their prior information about the speaker to resolve ambiguity in the utterance.
...
p3 <- dex3 %>%
  mutate(common_ground_manipulation = paste0(toupper(substr(common_ground_manipulation, 1, 1)), substr(common_ground_manipulation, 2, nchar(common_ground_manipulation))),
         speaker = paste0(toupper(substr(speaker, 1, 1)), substr(speaker, 2, nchar(speaker))),
         speaker = str_replace(speaker, "_", " "),
         alignment = paste0(toupper(substr(alignment, 1, 1)), substr(alignment, 2, nchar(alignment))))%>%
...
ex3_tab <- dex3 %>%  
  group_by(speaker ,common_ground_manipulation,alignment, id) %>%
  summarise(correct_inf = mean(correct_inf)) %>%
  summarize(correct_inf = list(correct_inf)) %>%
  group_by(speaker ,common_ground_manipulation,alignment) %>%
  ...
  select(speaker ,common_ground_manipulation,alignment, M , W,p)%>%
  mutate(M = round(M,2),
...
A range probabilistic frameworks been used to model word learning [e.g. @fazly2010probabilistic; @frank2009using; @xu2007word]. RSA models differ from other approaches in that they treat word learning as the outcome of a social reasoning process. In contrast to models for cross-situational word learning [@fazly2010probabilistic; @frank2009using], RSA models show how learning might occur in a one shot scenario based on pragmatic reasoning alone. While the ad hoc informativeness inference characteristic for RSA would be predicted by the model of Xu and Tenenbaum [-@xu2007word], in their work it follows from the "size principle" of generalization [@tenenbaum2001generalization] and not from social reasoning. In cotrast to RSA, this approach does not offer a straightforward way to incorporate other types of social information such as expectations following from common ground.

We treated common ground expectations as equivalent to more basic manipulations of contextual salience [e.g. in @frank2012predicting] and did not explicitly model the social-cognitive processes that give rise to these expectations. The interaction around the object prior to the test event simply increased the probability that this particular speaker will refer to the object subsequently. The same change could be brought about if one of the objects would be made perceptually more salient, for example by making it flash. In future work, it would be interesting to explore ways to model common ground expectations explicitly as well as to contrast perceptual and interactional salience.
