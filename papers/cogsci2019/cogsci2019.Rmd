...
pred_ex3 <- readRDS(file = "saves/model_predictions_ex3_1.rds")%>%ungroup()
...
A range of probabilistic models have been used to model word learning [e.g. @fazly2010probabilistic; @frank2009using; @xu2007word]. RSA models differ from these approaches in that they treat word learning as the outcome of a social reasoning process. In contrast to models for cross-situational word learning [@fazly2010probabilistic; @frank2009using], RSA models show how learning might occur in a one shot scenario based on pragmatic reasoning alone. While the ad-hoc informativeness inference characteristic for RSA would be predicted by the model of Xu and Tenenbaum [-@xu2007word], in their work it follows from the "size principle" of generalization [@tenenbaum2001generalization] and not from social reasoning. In contrast to RSA, this approach does not offer a straightforward way to incorporate other types of social information such as expectations following from common ground.
