...
We assume that the intentional goal of the speaker is to get the listener to get an option of the correct type. That is, the informativity of an utterance is calculated with respect to conveying the correct object type.
First, we calculate the literal listener's posterior distribution over referents and lexica, and marginalize (average) over objects of the same type and the different lexica. 
...
$$
\begin{aligned}
& P_{L_0}(r_p, \mathcal{L}_1 | {u_3}) = P_{L_0}(r_p^1, \mathcal{L}_1 | {u_3})  + P_{L_0}(r_p^2, \mathcal{L}_1 | {u_3})  = 0+ 0.35 =0.35 \\
& P_{L_0}(r_y, \mathcal{L}_1 | {u_3}) = P_{L_0}(r_y^2, \mathcal{L}_1 | {u_3})  = 0 \\
& P_{L_0}(r_p, \mathcal{L}_2 | {u_3})  = P_{L_0}(r_p^1, \mathcal{L}_2 | {u_3})  + P_{L_0}(r_p^2, \mathcal{L}_2 | {u_3})  = 0+ 0 =0 \\
& P_{L_0}(r_y, \mathcal{L}_2 | {u_3}) = P_{L_0}(r_y^2, \mathcal{L}_2 | {u_3}) = 0.65\\
\end{aligned}
$$
...
$$
\begin{aligned}
&P_{L_0}(r_p|u_3) =  P_{L_0}(r_p, \mathcal{L}_1|u_3) + P_{L_0}(r_p, \mathcal{L}_2|u_3) = 0.35+0=0.35 \\
&P_{L_0}(r_y|u_3) =  P_{L_0}(r_y, \mathcal{L}_1|u_3) + P_{L_0}(r_y, \mathcal{L}_2|u_3) = 0+0.65=0.65 \\
\end{aligned}
$$
...
\end{aligned}
$$
We assume that the speaker knows the lexicon, but doesn't believe the listener to know the lexicon. That is, the generative process of the utterance is tantamount to pointing to the table with the referent of the type they want and incidentally labeling it. The label itself carries no information. 

We can now compute the probability with each a speaker would produce each of the 4 utterances given the goal of conveying each of the 2 types.

$$
P_{S_1}(u|r) = \frac{\exp(\alpha \cdot Informativity(u;r))}{\sum_{u'} \exp(\alpha \cdot Informativity(u';r))}
$$

We begin with $r_p$. If the lexicon was $\mathcal{L}_1$, the speaker would say ...

$$
P_{S_1}(u|r^1_p) = \frac{\exp(\alpha \cdot Informativity(u;r_p))}{\sum_{u'} \exp(\alpha \cdot Informativity(u';r_p))}
$$
$$
\exp(\alpha \cdot Informativity(u;r_p)) = \begin{cases} 
\exp(2.24 \times 0) = 1 & \mbox{for } u_1 \\
\exp(2.24 \times 0) = 1& \mbox{for } u_2 \\
\exp(2.24 \times -\infty) = 0  & \mbox{for } u_3 \\
\exp(2.24 \times -\infty) = 0& \mbox{for } u_4 \\
\end{cases}
$$

To arrive at the speaker production probabilities, we divide by the normalizing constant (the sum of all of the unnormalized probabilties):

$$
P_{S_1}(u|r^1_p) = \begin{cases} 
0.5  & \mbox{for } u_1 \\
0.5 & \mbox{for } u_2 \\
 0  & \mbox{for } u_3 \\
0& \mbox{for } u_4 \\
...
$$
\exp(\alpha \cdot Informativity(u;r^2_p)) = \begin{cases} 
\exp(2.24 \times -\infty) = 0 & \mbox{for } u_1 \\
\exp(2.24 \times -\infty) = 0& \mbox{for } u_2 \\
\exp(2.24 \times -1.04) = 0.1  & \mbox{for } u_3 \\
\exp(2.24 \times -1.04) = 0.1 & \mbox{for } u_4 \\
\end{cases}
$$
...
$$
\exp(\alpha Informativity(u_3;r)) = \exp(2.24 \times)
$$

Now, to compute $P_{S_1}(u_3|r_3)$, we need the informativity for $r_3$ given the other utterance with the same label. We need

$$
\begin{aligned}
& P_{L_0}(r_3^y, \mathcal{L}_1 | {u_1}) = 0 \times 0.48 \times 0.5 = 0\\
& P_{L_0}(r_3^y, \mathcal{L}_2 | {u_1}) = 0 \times 0.48 \times 0.5 = 0\\
\end{aligned}
$$
and 
...
$$
\begin{aligned}
&P_{L_0}(r_3|u_1) =  P_{L_0}(r_3, \mathcal{L}_1|u_1) + P_{L_0}(r_1, \mathcal{L}_2|u_1) = 0+0=0 \\
\end{aligned}
$$
as well as
...
$$
\begin{aligned}
&Informativity(u_1; r_3) = log(0) = -\infty \\
\end{aligned}
$$
...
$$
P_{S_1}(u_3|r_3) = \frac{exp(2.24 \times -1.43)}{exp(2.24 \times -1.43)+exp(2.24 \times -\infty)}=1
$$
Thus, the probability that the speaker will chose $u_3$ to refer to $r_3$ is 1. We can now turn to equation 1 to get the probability with which the pragmatic listener ($L_1$) think that the Speaker ($S_1$) is referring to $r_3$ when producing utterance $u_3$.

MANUEL: THE ABOVE HAS TO BE TRUE, AT LEAST THIS IS THE VALUE WE GET OUT OF THE WEBPPL MODEL.



To get the probability with which the pragmatic listener $L_1$ think that the $S_1$ is referring to $r_3$ when producing utterance 
