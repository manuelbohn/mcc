...
There are two kinds of lexica, corresponding to the two utterance components: The lexicon of pointing $\mathcal{L}_{point}$ and the lexica of label $\mathcal{L}_1, \mathcal{L}_2$.
The lexicon of pointing $\mathcal{L}_{point}$ encodes the meaning of a point: The action of turning (pointing) reduces the set of referents to those that are on the table targeted by the turn (point).
The lexica of labels map labels $u_w$ onto referents: $\mathcal{L}_1$ maps label 1 $w_1$ to type 1 $t_1$ and label 2 $w_2$ to type 2 $t_2$, and $\mathcal{L}_2$ which has the inverse mapping. 
...
The right-hand side of this equation has three terms: the prior distribution over referents $P(r)$, the prior distribution over lexica $P(\mathcal{L})$, and the likelihood that a speaker would produce an utterance $u$ given a referent type $r_t$ and their lexicon $\mathcal{L}$: ($P_{S1}(u|r_t, \mathcal{L})$).
...
The speaker's utterances are (action, label) pairs $(u_a, u_l)$ (i.e., we assume the speaker must turn to one of the locations and must produce a label). The label the speaker produces $u_l$ depends upon their lexicon $\mathcal{L}$ (i.e., they choose the label that is consistent with the object under their lexicon). The action (a point or a turn) the speaker produces $u_a$ is a function of that signal's informativity. That is, because the listener does not know the meaning of the labels, the labels carry no information to the listener.
...
\noindent where the probability that the naive listener $L_0$ would select that a referent of a given type $r_t$ given the utterance $u_a$ is given by Bayes' Rule:
...
because the speaker is pointing to the table with $r_{p2}$ and $r_{y2}$.
...
$$
\begin{aligned}
& P_{L_0}(r_p | {u_3}) = P_{L_0}(r_{p1}| {u_3})  + P_{L_0}(r_{p2}| {u_3})  = 0+ 0.35 =0.35 \\
& P_{L_0}(r_y | {u_3}) = P_{L_0}(r_{y2} | {u_3}) = 0.65\\
\end{aligned}
$$
...
$$
\begin{aligned}
&Informativity(u_3; r_p) = \log(0.35) = -1.04 \\
&Informativity(u_3; r_y) = \log(0.65) = -0.43
\end{aligned}
$$
...
$$
\begin{aligned}
&Informativity(u_1; r_p) = log(P_{L_0}(r_p|u_1)) = \log(1)  = 0\\
&Informativity(u_1; r_y) = log(P_{L_0}(r_y|u_1)) = \log(0) = -\infty
\end{aligned}
$$
