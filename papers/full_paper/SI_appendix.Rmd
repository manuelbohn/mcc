...
The situation we model is defined by three sets: referents $r$, utterances $u$, and lexica $L$. 
Referents are defined two features: the type of object they are (visually discernible given their shape and color) and their location (a table).
There are two types $t_1$, $t_2$ and two locations $l_1$ and $l_2$.
...
The action of the turning or pointing reduces the set of referents to those that are on the table targeted by the turn (point) and interpreting a label reduces that set further to that which is of the type referred to by that label.
The interpretation of a label involves a lexicon $L$, which provides the label--type mappings.
...
The pragmatics model is a Rational Speech Act model that captures the following process: A pragmatic listener ($L_1$) jointly infers a referent (what object is being picked out by the utterance) and a lexicon (what does the utterance mean) by reasoning about a pragmatic speaker ($S_1$) who produces utterances to convey information to a literal listener ($L_0$), who in turn interprets utterances according to their literal meaning. According to Bayes rule, this inference is given by:

\begin{equation} 
P_{L_1}(r,L|u)\propto P_{S_1}(u|r)P(L)P(r)
\end{equation} 

The right side of this equation has three components: the prior distribution over referents $P(r)$, the prior distribution over lexica (word - object mappings) $P(L)$, and the likelihood that a speaker would produce the an utterance $u$ given a referent $r$ ($P_{S1}(u|r)$).
In the situation we model, the prior on referents $P(r)$ is a categorical distribution over three objects in a scene, which we assume could be non-uniform due to our common ground manipulations (see below). 
Objects are defined a single feature: the type of object they are. 
...
the listener's prior has no substantive knowledge of the meanings of the labels that a speaker could produce.
...
In the following, we walk through a numerical example for how model predictions were generated for the pragmatics model. The prediction will correspond to the parameter free model, excluding the noise parameter. The numbers are taken from the preference - different speaker - congruent condition. Fig. 1 shows a screenshot from the adult experiment, which the model was designed to capture. In this context, there are three potential referents (two red-ish: $r_1^r$, $r_2^r$; one yellow-ish, $r_3^y$,), of two types (red-ish and yellow-ish) on two tables. In principle, the speaker, the frog in this case, can perform one of two utterances: $u_1$, which is turning to the right table (two objects) and making a request by using a novel label or $u_2$, which involves turning to the left table (single object) and making a request by using a novel label. In the experiment, the speaker chose $u_1$ and we therefore model $L_1$'s interpretation of this utterance.
...
From a learning perspective, we assume that the listener is learning the mappings between labels and object types instead of object tokens. That is, the listener either thinks the novel word refers to "red-ish objects" or "yellow-ish objects". 

The prior distribution over objects (left to right in Fig.1), based on Experiment 2A, for this condition was [`r round(1-mean(adult_ex2_preference_data%>%filter(condition == "different_speaker")%>%pull(correct)),2)`; `r round(1-mean(adult_ex2_preference_data%>%filter(condition == "different_speaker")%>%pull(correct)),2)`; `r round(mean(adult_ex2_preference_data%>%filter(condition == "different_speaker")%>%pull(correct)),2)`]. That is, the yellow object has a higher a priori probability compared to red objects, because the speaker previously displayed a preference for a yellow object. The informativity of the observed utterance for each referent (red or yellow object) can be calculated based on equation 3. 

$$
\begin{aligned}
&Informativity(u_1; r_1) = log(0.64 \cdot 1) = -0.44 \\
&Informativity(u_1; r_2) = log(0.36 \cdot 1) = -1.03
\end{aligned}
$$

with $r_1$ being the yellow object and $r_2$ being the red object. Next we can insert these values into equation 2 to compute the probability that the speaker chooses this utterance to refer to this type of object. To do so, we need the speaker optimality parameter of $\alpha = 2.24$, which was estimated based on Experiment 1.

$$
\begin{aligned}
&P_{S_1}(u_1|r_1) = \frac{exp(2.24 \cdot -0.44)}{exp(2.24 \cdot -0.44)} = 1 \\
&P_{S_1}(u_1|r_2) = \frac{exp(2.24 \cdot -1.03)}{exp(2.24 \cdot -1.03) + exp(2.24 \cdot 0)} = 0.09
\end{aligned}
$$

Here, the informativity for a referent is normalized across all utterances that could be used to refer to this referent. Finally, based on equation 1 we can use these values to compute the probability that the speaker thinks that the speaker is referring to the yellow object ($r_1$) when they produce $r_1$:
