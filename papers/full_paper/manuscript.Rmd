---
title             : "Predicting the integration of social information in pragmatic word learning across development"
...
Language use as and learning  integration of different sources of information. Next to conventional syntax and semantics, expectations about communicative interactions play a decisive role. These expectations can be considered on two levels. on the one hand, general expectations that apply to any interlocutor. On top of this there is information that is particular to a specific speaker which is included in the common ground between interlocutors. This generates specific expectations .  

Knowledge of communicative conventions such as words in a language could also be considered as (cultural) common ground information, generating certain expectations about communicative behavior

In practice, communication requires balancing and integrating these different sources of information. RSA bla bla

From a developmental perspective

took a truyl developmental perspective in a
...
Participants made the inference consistent with the assumption that the speaker communicated informatively: They selected object B above chance (0.5) in the test condition (t(`r ex1t%>%filter(condition == "test")%>%pull(df)`) = `r ex1t%>%filter(condition == "test")%>%pull(t_value)`, *p* `r ex1t%>%filter(condition == "test")%>%pull(p_value)`, see Fig. 2) and more often compared to the control condition (*$\beta$* = `r ex1_r%>%filter(term == "conditiontest")%>%pull(estimate)`, se = `r ex1_r%>%filter(term == "conditiontest")%>%pull(std.error)`, *p* `r ex1_r%>%filter(term == "conditiontest")%>%pull(p.value)`). 
...
prior as change in salience

Development is reflected as change in each expectation, the process of integration is assumed to be the same. this could be changed in the future 

bring down noise level for 3yo when testing between subjects?
