...
The following sections present details on the analyses used to evaluate whether the different experimental manipulations produced different responses. For all generalized linear mixed models (GLMM), we included random intercepts for participant id (`id`) and the animal speaker (`agent` - representing the respective "item") with random slopes for condition (same speaker vs. different speaker), alignment (congruent vs. incongruent; experiment 3, 4 and 7) and age (experiment 5-7). We also included interactions between random effects. This maximal structure was reduced until the model converged. The formula of the final model is presented in the text. To access further details about GLMMS, including variance estimates for random effects, please download the online repository, open the file `papers/full_paper/Supplementary_information.Rmd` and run the command `summary()` on the respective object that stores the output of each model. 

We used `r cite_r("library.bib")` for all our analyses reported in the main manuscript and the supplementary material.

## Experiment 1

### Procedure

```{r figS1, include = T, fig.align = "center", fig.cap = "Screenshot showing test situation Experiment 1, 3 and 4.", out.width="100%"}
...
```{r figS2, include = T, fig.align = "center", fig.cap = "Screenshot showing test situation in Experiment 2 for adults.", out.width="100%"}
...
```{r plotex3, fig.cap = "Data and model predictions by condition for Experiment 3 with adults. Transparent dots show data from individual participants, diamonds represent condition means. Empirical means are shonw in black and model predictions in pink. Error bars represent 95\\% HDIs.", fig.align = "center", out.width="100%"}
...
We discuss and visualize the results as the proportion with which participants chose the more informative object (i.e., the object that would be the more informative referent when only utterance information is considered). Figure\ref{fig:plotex3} shows the result in each of the eight conditions of Experiment 3. Results are plotted from  The plot also shows model predictions from the integration model (including the noise parameter - see below).
...
ex4_prior_models <- adult_ex4_prior_data %>%
  ...
  do(broom::tidy(glmer(correct ~ speaker + (1|id) + (1|agent),data = .,family=binomial,  control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))))
  


ex4_prior_models_table <- ex4_prior_models%>%
  filter(effect == "fixed")%>%
  select(-statistic, -group, -effect)%>%
  mutate(p.value = ifelse(p.value<.001,"< .001",as.character(paste("=",substr(round(p.value,3),2,5),sep=" "))),
         std.error = round(std.error,2),
         estimate = round(estimate,2),
...
```{r priorplot, fig.cap = "Results from prior strength manipulation Experiments. Transparent dots show data from individual participants, diamonds represent condition means, error bars are 95\\% CIs. Dashed line indicates performance expected by chance.", fig.align = "center", out.width="100%"}
...
lm_ex4_r_strong <- broom.mixed::tidy(lm_ex4_strong, effects = "fixed")%>%
  mutate(p.value = ifelse(p.value<.001,"< .001",as.character(paste("=",substr(round(p.value,3),2,5),sep=" "))),
         std.error = round(std.error,2),
         estimate = round(estimate,2))%>%
  as_tibble()%>%
...
lm_ex4_r_medium <- broom.mixed::tidy(lm_ex4_medium, effects = "fixed")%>%
  mutate(p.value = ifelse(p.value<.001,"< .001",as.character(paste("=",substr(round(p.value,3),2,5),sep=" "))),
         std.error = round(std.error,2),
         estimate = round(estimate,2))%>%
  as_tibble()%>%
...
lm_ex4_r_weak <- broom.mixed::tidy(lm_ex4_weak, effects = "fixed")%>%
  mutate(p.value = ifelse(p.value<.001,"< .001",as.character(paste("=",substr(round(p.value,3),2,5),sep=" "))),
         std.error = round(std.error,2),
         estimate = round(estimate,2))%>%
  as_tibble()%>%
...
Results from Experiments 3 and 4 (strong prior manipulation) were highly correlated (r = `r cor_3_4$estimate`, *p* > .001). 
...
```{r figS3, include = T, fig.align = "center", fig.cap = "Screenshot showing test situation Experiment 5 and 7.", out.width="100%"}
...
ggplot(data = p_child_ex12, aes(x = age_num, y = correct, col = condition)) +
  ...
  geom_smooth(aes(fill = condition),method = "glm",method.args = list(family = "binomial"), se = T, alpha = .5, size = .7)+
...
```{r child_models, results = "asis", out.width="100%"}
...
child_model_predictions <- readRDS("../../stats/saves/child_ex3_model_predictions.rds")%>%
  mutate(speaker = ifelse(speaker == "different_speaker", "Different speaker", "Same speaker"),
         alignment = ifelse(alignment == "congruent", "Congruent", "Incongruent"))%>%
  mutate(model = recode(model, pragmatic = "Parameter free", pragmatic_developmental_noise = "Developmental noise", pragmatic_noise = "Noise"))
...
p_child_ex3_data_binned <- child_ex3_data %>%
...
ggplot(child_model_predictions)+
  ...
  geom_smooth(data = p_child_ex3_data, aes(x = age_num, y = correct_inf), col = "black", size = 0.6, method = "glm",method.args = list(family = "binomial"),  se = T,  alpha = .5)+
geom_pointrange(data = p_child_ex3_data_binned, aes(x = as.numeric(age)+0.5, y = mean, ymin = ci_lower, ymax = ci_upper), size = .6,  position = position_dodge(width = .5), pch = 5)+
  ...
  scale_color_manual(name="Model",
                     breaks=c("Parameter free","Noise","Developmental noise"),
                     values= c("#00afaf","#6c71c4","#af6300"))+
...
Figure \ref{fig:plotmodelpredex7} shows the results for each condition plotted by age as a continuous variable. The same figure also shows model predictions from the integration model with different noise parameters (see below).
...
To conclude the example, according to the parameter free integration model, the probability that $L_1$ thinks that $S_1$ is referring to a yellow-ish object when producing $u_3$ is 0.91. Based on this model we thus expected that, on average, participants would select the yellow-ish object in 91% of cases in a condition with the prior distribution specified above.  

## No informativeness model

The No informativeness model ignored the information about the intended referent that was expressed by the utterance and instead only focused on common ground manipulation. The only information available to $L_1$ is the prior distribution over referents. It is therefore defined as: 

\begin{equation} 
P_{L_1}(r|u)\propto P(r)
\end{equation} 

That is, the probability of the referent given the utterance is determined by the prior probability of the referent for a particular speaker. The prior distributions were set in the same way as for the integration model.

## No conversational prior model

This model was identical in structure to the integration model with the exception that the prior distribution did not correspond to the measurements from Experiment 2 and did not vary with speaker identity. That is, regardless of common ground manipulation and speaker identity the prior distribution was always uniform (i.e. [0.33,0.33,0.33]). The speaker optimality parameter $\alpha$ was set in the same way as in the integration model. 

# Model parameters  

As noted in the main text, the parameter $\alpha$ (speaker optimality parameter) in equation 2 determines the absolute strength of the likelihood term. Its interpretation is *how* rational $L_1$ thinks $S_1$ is in this particular context. For adults, we used the data from Experiment 1 to infer $\alpha$. That is, we performed a Bayesian analysis in which we used the integration model (assuming equal prior probability over referents), with an unknown *a priori* value of $\alpha$, and conditioned on the data from Experiment 1 to compute a posterior distribution over $\alpha$; in turn, the model generates posterior predictions for the proportion of correct responses in Experiment 1. We used MCMC to approximate the posterior distribution for $\alpha$ based on Experiment 1, computed the *maximum a posteriori* (MAP) estimate and used this value for $\alpha$ to generate model predictions for Experiment 3 and 4.	The prior distribution was $\alpha \sim {\rm Cauchy }(1,2)$. We collected samples from four independent MCMC chains, collecting 6000 samples from each chain and removing the first 2000 for burn-in. 

For children, $\alpha$ changed with age. Thus, $\alpha$ for a given age was defined via a simple linear regression and instead of inferring a single value across age, we used the data from Experiment 5 to find the intercept ($\beta^\alpha_0$) and slope ($\beta^\alpha_1$) that best described the developmental trajectory in the data. As for adults, this was done via the integration model with equal prior probability for each object. We performed a Bayesian analysis to compute a posterior distribution for the intercept and the slope of this regression. The prior distribution was $\beta^\alpha_0 \sim {\rm Cauchy }(1,2)$ for the intercept and $\beta^\alpha_1 \sim {\rm Cauchy }(0,2)$ for the slope. We collected samples from four independent MCMC chains, collecting 6000 samples from each chain and removing the first 2000 for burn-in. In Experiment 7, the speaker optimality parameter for a given child of a given age was computed by taking the MAP for the intercept and adding the MAP for the slope times the child's age (with age anchored at 0): $\alpha_i = \beta^\alpha_0 + i \cdot \beta^\alpha_1$.

The prior distribution over objects, $P(r)$, varied with the common ground manipulation, the identity of the speaker and the alignment of utterance and common ground information. Numerically, these probabilities depended on the measurement obtained in Experiment 2A and B for adults and Experiment 6 for children. Fig. \ref{fig:figS2} shows the test situation on Experiment 2.  

For adults, the prior over objects was estimated in the following way: For example, in Experiment 2, for the preference/same speaker condition, when the speaker previously indicated that they liked the yellow-ish object (right table in Fig. \ref{fig:figS2}) and disliked the pink-ish object (left table in Fig.\ref{fig:figS2}), the average proportion with which participants chose the yellow-ish object  was `r adult_ex2_preference_data %>% filter(condition == "same_speaker") %>% summarise(mean = mean(correct)) %>% pull(mean)` and for the pink-ish object it was `r 1-adult_ex2_preference_data %>% filter(condition == "same_speaker") %>% summarise(mean = mean(correct)) %>% pull(mean)` respectively. In Experiments 3 and 4, this measurement determined the prior distribution over objects in cases whenever the same manipulation was used (preference/same speaker). Note that Experiment 3 involved three objects while Experiment 2 only involved two. We nevertheless used the exact proportions measured in Experiment 2 for each object as unnormalized probabilities in the prior. This approach  conserved the relative relation between object types. Thus, when utterance and common ground information were aligned (i.e. the yellow-ish object was the more informative object as in Fig. \ref{fig:figS1}), the distribution of objects was [$r_{p1}, r_{p2}, r_{y2}$]. Using the raw proportions, the corresponding prior distribution was [$P(r_{p}^1) = 0.03, P(r_{p}^2) = 0.03, P(r_{y}^2) = 0.97$] and after normalizing (so that they add up to 1) it was [0.03, 0.03, 0.94]. When information sources were dis-aligned (i.e. the pink-ish object was the more informative one), the object distribution was [$r_{y1}, r_{p2}, r_{y2}$] and the prior distribution was thus [0.97, 0.03, 0.97] or [0.49, 0.02, 0.49] after normalizing.

For children, we used the data from Experiment 6 to model the intercepts ($\beta^\rho_{0,j}$) and slopes ($\beta^\rho_{1,j}$) that best described the developmental trajectory in the data for each of the two ($j$) conditions. As for the speaker optimality parameter, this allowed us to generate object prior distributions in the cognitive model that were sensitive to the child's age. We used a simple Bayesian logistic regression to find the intercept and slope (MAP of posterior distribution) that best described children's performance in the two conditions of experiment 6. The prior distribution was $\beta^\rho_0 ,\beta^\rho_1 \sim \text{Uniform}(-2,2)$ for the intercept and slope. We collected samples from four independent MCMC chains, collecting 6000 samples from each chain and removing the first 2000 for burn-in. In Experiment 7, the prior probability for an object was computed by taking the intercept for the respective condition ($j$), adding the slope times the child's age ($i$) and then using a logistic transformation to convert the outcome into proportions: $\rho_{i,j} = \text{logistic}(\beta^\rho_{0,j} + i \cdot \beta^\rho_{1,j})$. Because these proportions corresponded to a two object scenario, they were then converted to the three object scenario via normalization in the same way as for adults. The overall distribution then depended on the alignment of information sources in the same way as it did for adults. Model code for inferring the parameters can be found in the associated online repository in the file `models.Rmd` starting in line 213 (adults) 1464 (children).  
...
Here we report details on the model comparisons. Model fit was assessed based on marginal log-likelihoods of the data under each model. To compute the log-likelihood for models including a noise parameter, we used forward sampling on four independent chains to collect 5000 samples from the prior distribution of the noise parameter, calculated the log-likelihood for each sample and then averaged over the resulting distribution. 

Bayes Factors were then computed by first subtracting log-likelihoods for two models and then exponentiating the result. Table \ref{tab:ex3_comp} shows Bayes Factors for model comparisons in Experiment 3. We did not pre-register the inclusion of the noise parameter for Experiment 3, but did so for all subsequent experiments for which we did model comparisons (4 and 7). The first row in Table \ref{tab:ex3_comp} compares the integration model with noise parameter to the model without the noise parameter. This comparison shows that including the noise parameter greatly improves model fit. Figure \ref{fig:modelpredex3} correlates model predictions from the models including noise parameters to the data from Experiment 3. 

```{r modelpredex3, include = T, fig.cap = "Correlation plot for model predictions and data from Experiment 3. All models depicted here included a noise parameter. Coefficients and p-values are based on Pearson correlation statistics. Dots represent condition modes. Error bars represent 95\\% HDIs.", fig.align = "center", out.width="100%"}
...
ex3_flat_prior_model_pred_noise <- readRDS("../../stats/saves/ex3_flat_prior_model_noise.rds") %>%
  filter(!(Parameter %in% c("noise")))  %>%
  separate(Parameter, into = c("common_ground_manipulation","speaker", "alignment"), sep="/")%>%
  mutate(model="no_common_ground_noise")%>%
  group_by(model,common_ground_manipulation,speaker,alignment)%>%
  summarise(mean = estimate_mode(value), 
            ci_lower = hdi_lower(value),
            ci_upper = hdi_upper(value))
...
ex3_prior_only_model_pred_noise <- readRDS("../../stats/saves/ex3_prior_only_model_noise.rds") %>%
  filter(!(Parameter %in% c("noise")))  %>%
  separate(Parameter, into = c("common_ground_manipulation","speaker", "alignment"), sep="/")%>%
  mutate(model="no_informativeness_noise")%>%
  group_by(model,common_ground_manipulation,speaker,alignment)%>%
  summarise(mean = estimate_mode(value), 
            ci_lower = hdi_lower(value),
            ci_upper = hdi_upper(value))
...
ex3_pragm_model_pred_noise <- readRDS("../../stats/saves/ex3_pragm_model_noise.rds") %>%
  filter(!(Parameter %in% c("noise")))  %>%
  separate(Parameter, into = c("common_ground_manipulation","speaker", "alignment"), sep="/")%>%
  mutate(model="integration_noise")%>%
  group_by(model,common_ground_manipulation,speaker,alignment)%>%
  summarise(mean = estimate_mode(value), 
            ci_lower = hdi_lower(value),
            ci_upper = hdi_upper(value))
...
ex_3_cor_plot <- ex_3_pred %>%
  ...
  mutate(model =  case_when(model == "integration_noise" ~ "Integration",
                            model == "no_informativeness_noise" ~ "No informativeness",
                            model == "no_common_ground_noise" ~ "No conversational prior"
                            ))
...
ex3_bf <- bind_rows(
  ...
  mutate("integration_noise > integration" = exp(integration_noise - integration),
         "integration_noise > no_informativeness_noise" = exp(integration_noise - no_informativeness_noise),
         "integration_noise > no_common_ground_noise" = exp(integration_noise - no_common_ground_noise),
         "no_informativeness_noise > no_common_ground_noise" = exp(no_informativeness_noise - no_common_ground_noise)) %>%
  select(-integration,-integration_noise, -no_informativeness_noise, -no_common_ground_noise)%>%
...
```{r ex3noise, include = T, fig.cap = "Posterior distribution of noise parameter for each model in Experiment 4", fig.align = "center", out.width="80%", fig.height=3}
ex3_noise_parameters <- bind_rows(
  ...
  filter(Parameter %in% c("noise"))  %>%
  mutate(Model = "No informativeness"),
  readRDS("../../stats/saves/ex3_flat_prior_model_noise.rds") %>%
  filter(Parameter %in% c("noise"))  %>%
  mutate(Model = "No conversational prior")
  )
...
ggplot(ex3_noise_parameters, aes(x = value, col = Model, fill = Model))+
  ...
  xlim(0,1)+
  scale_color_manual(breaks=c("Integration","No informativeness","No conversational prior"),
                    values= c("#C5D86D","#86615C","#AFE0CE"))+
  scale_fill_manual(breaks=c("Integration","No informativeness","No conversational prior"),
                    values= c("#C5D86D","#86615C","#AFE0CE"))
```

Figure \ref{fig:ex3noise} shows the posterior distribution of the noise parameter under each model. The noise parameter was fit to the data and indicates the proportion of responses that are estimated to be due to random guessing rather than in line with model predictions. Consequently, a model that makes predictions that are closer to the data is likely to have a lower noise parameter. The results corroborate the model comparison by showing that the integration model has the lowest noise parameter.  

## Experiment 4

```{r modelpredex4, include = T, fig.cap = "Correlation plot for model predictions and data from Experiment 4. All models included a noise parameter. Coefficients and p-values are based on Pearson correlation statistics. Dots represent condition modes. Error bars represent 95\\% HDIs.", fig.align = "center", out.width="100%"}
...
ex4_prior_only_model_pred_noise <- readRDS("../../stats/saves/ex4_prior_only_model_noise.rds") %>%
  filter(!(Parameter %in% c("noise")))  %>%
  separate(Parameter, into = c("common_ground_manipulation","prior_manipulation","speaker", "alignment"), sep="/")%>%
  mutate(model="no_informativeness")%>%
  group_by(model,common_ground_manipulation,prior_manipulation,speaker,alignment)%>%
  summarise(mean = estimate_mode(value), 
            ci_lower = hdi_lower(value),
            ci_upper = hdi_upper(value))
...
ex_4_cor_plot <- ex_4_pred %>%
  ...
  mutate(model =  case_when(model == "integration_noise" ~ "Integration",
                            model == "no_informativeness" ~ "No informativeness",
                            model == "no_common_ground" ~ "No conversational prior"
                            ))
...
ex4_bf <- bind_rows(
  ...
  mutate("Integration > No informativeness" = exp(integration_noise - no_informativeness),
         "Integration > No conversational prior" = exp(integration_noise - no_common_ground),
         "No conversational prior > No informativeness" = exp(no_common_ground - no_informativeness)) %>%
  select(-integration_noise, -no_common_ground, -no_informativeness)%>%
  gather(Comparison, BF)%>%
  mutate(BF = formatC(BF,2))
...
```{r ex4noise, include = T, fig.cap = "Posterior distribution of noise parameter for each model in Experiment 4", fig.align = "center", out.width="80%", fig.height=3}
...
apa_table(
  ...
  , caption = "Model comparisons for integration models in Experiment 7"
  , note = "BF = Bayes Factor"
  , escape = T
)
```

Table \ref{tab:child_bf} shows results for model comparison for the different types of noise parameters. In all cases, the integration model provides a substantially better fit to the data compared to the alternative models.

```{r child_bf, results = "asis", out.width="100%"}
child_ex3_bf <- child_ex3_model_comparison%>%
  select(-ci_lower, -ci_upper)%>%
  rename(Parameter = parameter)%>%
  group_by(Parameter)%>%
  spread(model, logP) %>%
  mutate("Int > No CP" = formatC(exp(pragmatic - flat_prior),2),
         "Int > No Inf" = formatC(exp(pragmatic - prior_only),2),
         "No CP > No Inf" = formatC(exp(flat_prior - prior_only),2)) %>%
  select(-pragmatic,-flat_prior, -prior_only)

apa_table(
  child_ex3_bf
  , caption = "Model comparisons in Experiment 7"
  , note = "BF = Bayes Factor, Int = Integration, No CP = No conversational prior, No Inf = No informativeness "
  , escape = T
)
```
...
developmental_noise_parameter <- data_frame(
  ...
  Model = c(rep("Integration",41),
            rep("No conversational prior",41),
            rep("No informativeness",41)),
...
Figure \ref{fig:childnoise} shows the different types of noise parameters for the each model. Figure \ref{fig:childnoise}A shows that the integration model has the lowest estimated level of noise of all the models considered. Figure \ref{fig:childnoise}B shows that the the integration model has the lowest level of estimated noise across the entire age range. It also shows that noise decreases with age for the integration model, suggesting that older children behaved more in line with model predictions compared to younger children. 

```{r childnoise, include = T, fig.cap = "Posterior distribution of noise parameter for each model in Experiment 7. A: single noise parameter across age, B: Developmental noise parameter.", fig.align = "center", out.width="100%", fig.height=3}
ggarrange(child_noise_plot,child_dev_noise_plot, nrow = 1, ncol = 2, common.legend = T, legend = "right", labels = c("A","B"))
```

Finally, Figure \ref{fig:childcor} shows correlations between model predictions and the data, binned by year. Across noise parameters, model predictions and data are closest aligned (i.e. closest to the dotted line) for the integration model, thereby corroborating the conclusions drawn based on the model comparison and the evaluation of the noise parameters. Correlations are also higher for 4yo compared to 3yo, supporting the interpretation based on the developmental noise parameter that children behaved more in line with the model predictions as they got older.  

```{r childcor,  include = T, fig.cap = "Correlation plot for model predictions and data for all models considered in Experiment 7. Dots represent condition modes. Error bars represent 95\\% HDIs.", fig.align = "center", out.width="100%", fig.height=7}
# child_cor <- bind_rows(
#   readRDS("../../stats/saves/child_ex3_pragm_model_noise.rds") %>%
#   filter(!(Parameter %in% c("noise"))) %>%
...
p_child_cor<- readRDS("../../stats/saves/child_all_models_cor.rds") %>%
  ...
  mutate(model = recode(model,
                          ...
                          "Flat prior" = "No conversational prior",
                          "Prior only" = "No informativeness"),
         data_mean = rep(p_child_ex3_data_binned%>%pull(mean),9),
         data_ci_upper = rep(p_child_ex3_data_binned%>%pull(ci_upper),9),
         data_ci_lower = rep(p_child_ex3_data_binned%>%pull(ci_lower),9))
...
In this model, we assume that the literal listener ($L_0$) is also sensitive to the prior manipulation. An alternative approach [used e.g. in @frank2012predicting] is to assume that $L_0$ has a uniform prior distribution over referents [.33, .33, .33]. We also ran the entire analysis with this alternative approach and found no major changes in the results. In all cases, the integration model outperformed the two alternative models, both for children as well as for adults. Interestingly, for adults but not for children, using a uniform prior for $L_0$ yielded a lower estimate for the noise parameter in Experiments 3 and 4. 
