...
As noted in the main text, the parameter $\alpha$ (speaker optimality parameter) in equation 2 determines the absolute strength of the likelihood term. It's interpretation is *how* rational $L_1$ thinks $S_1$ is in this particular con    text. For adults, we used the data from Experiment 1 to infer $\alpha$. That is, we inferred which value of $\alpha$ would generate model predictions for the integration model (assuming equal prior probability over referents) that corresponded to the average proportion of correct responses measured in Experiment 1. Technically, we used MCMC to compute the posterior distribution for $\alpha$ and computed the maximum a posteriori (MAP). This value for $\alpha$ was then used in Experiments 3 and 4. 

For children, the speaker optimality parameter changed with age. Instead of inferring a single value across age, we used the data from Experiment 5 to find the slope and intercept for $\alpha$ that best described the developmental trajectory in the data. As for adults, this was done via the integration model with equal prior probability for each object. In this model, $\alpha$ for a given age was defined via a simple linear regression. We calculated the MAP for the intercept and the slope of this regression based on their posterior distributions. In Experiment 7, the speaker optimality parameter for a given child of a given age was computed by taking the MAP for the intercept and adding the MAP for the slope times the child's age (with age anchored at 0).
