...
ex_3_cor_plot <- ex_3_pred %>%
  ...
  mutate(model =  case_when(model == "integration_noise" ~ "Integration",
                            ...
                            model == "no_common_ground_noise" ~ "No common ground"
                            ))
...
ggplot(ex3_noise_parameters, aes(x = value, col = Model, fill = Model))+
  ...
  xlim(0,1)+
  scale_color_manual(breaks=c("Integration","No informativeness","No common ground"),
                    values= c("#C5D86D","#86615C","#AFE0CE"))+
  scale_fill_manual(breaks=c("Integration","No informativeness","No common ground"),
                    values= c("#C5D86D","#86615C","#AFE0CE"))
```
...
ex_4_cor_plot <- ex_4_pred %>%
  ...
  mutate(model =  case_when(model == "integration_noise" ~ "Integration",
                            ...
                            model == "no_common_ground" ~ "No common ground"
                            ))
...
ex4_bf <- bind_rows(
  ...
  mutate("Integration > No informativeness" = exp(integration_noise - no_informativeness),
         "Integration > No common ground" = exp(integration_noise - no_common_ground),
         "No common ground > No informativeness" = exp(no_common_ground - no_informativeness)) %>%
...
child_ex3_bf <- child_ex3_model_comparison%>%
  ...
  group_by(Parameter)%>%
  spread(model, logP) %>%
  mutate("Int > No CG" = formatC(exp(pragmatic - flat_prior),2),
         "Int > No Inf" = formatC(exp(pragmatic - prior_only),2),
         "No CG > No Inf" = formatC(exp(flat_prior - prior_only),2)) %>%
  select(-pragmatic,-flat_prior, -prior_only)

apa_table(
  child_ex3_bf
  , caption = "Model comparisons in Experiment 7"
  , note = "BF = Bayes Factor, Int = Integration, No CG = No common ground, No Inf = No informativeness "
  , escape = T
)
```
