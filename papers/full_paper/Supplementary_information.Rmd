...
geom_pointrange(data = p_child_ex3_data_binned, aes(x = as.numeric(age)+0.5, y = mean, ymin = ci_lower, ymax = ci_upper), size = .6,  position = position_dodge(width = .5), pch = 5)+
  ...
  scale_color_manual(name="Model",
                     breaks=c("integration","integration_noise","integration_developmental_noise"),
...
As noted in the main text, the parameter $\alpha$ (speaker optimality parameter) in equation 2 determines the absolute strength of the likelihood term. It's interpretation is *how* rational $L_1$ thinks $S_1$ is in this particular context. For adults, we used the data from Experiment 1 to infer the value of $\alpha$. That is, we inferred which value of $\alpha$ would generate model predictions for the integration model (assuming equal prior probability over referents) that corresponded to the average proportion of correct responses measured in Experiment 1. Technically, we used MCMC to compute the posterior distribution for $\alpha$ and took the MAP. This value for $\alpha$ was then used in Experiment 3 and 4. 

For children, the speaker optimality parameter changed with age. Instead of inferring a single value across age, we used the data from Experiment 5 to find the slope and intercept (MAP values) for $\alpha$ that best described the developmental trajectory in the data. As for adults, this was done via the integration model with equal prior probability for each object. In Experiment 7, the speaker optimality parameter for a given child of a given age was computed by taking the overall intercept and adding the slope times the child's age (with age anchored at 0). The analysis code corresponding to these calculations can be found in the associated online repository in the file `models.Rmd`.
...
child_ex3_bf <- child_ex3_model_comparison%>%
  ...
  group_by(Parameter)%>%
  spread(model, logP) %>%
  mutate("Integration > No common ground" = formatC(exp(pragmatic - flat_prior),2),
         "Integration > No informativeness" = formatC(exp(pragmatic - prior_only),2),
         "No common ground > No informativeness" = formatC(exp(flat_prior - prior_only),2)) %>%
