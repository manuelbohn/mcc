...
# The prior on the speaker optimality parameter is largely positive but also contains negative values so that it could, in theory, result in a parameter that leads to model predictions below chance. The same prior is used for the intercept on children's parameters, where performance was below chance.  
speakOptParam <- '
var data = dataFromR.data

var model  = function(){

  var speakerOptimality = uniformDrift({a: -3, b: 7, width: 0.2})
...
```{r model setup ex 3 adults, include = FALSE}
...
```{r model rsa noise ex3 adults, include = FALSE}
predictions.rsa.ex3 <-'
...
```{r predictions pragmatic model ex 3 adults, include = FALSE}
...
```{r model prior only ex3 adults, include = FALSE}
...
```{r predictions prior only model ex3 adults, include = FALSE}
...
```{r model flat prior ex3 adults, include = FALSE}
...
ex3_bf <- data.frame(
  ...
  log_bayes_factor = c(
    log_like_pragmatic_ex3$model_log_like - log_like_flat_prior_ex3$model_log_like,
    log_like_pragmatic_ex3$model_log_like- log_like_prior_only_ex3$model_log_like,
    log_like_flat_prior_ex3$model_log_like - log_like_prior_only_ex3$model_log_like)
)
...
```{r model rsa noise ex4 adults, include = FALSE}
predictions.rsa.ex4 <-'
...
```{r predictions pragmatic model ex 4 adults, include = FALSE}
...
```{r model prior only ex4 adults, include = FALSE}
...
```{r predictions prior only model ex4 adults, include = FALSE}
...
```{r model flat prior ex4 adults, include = FALSE}
...
# In theory, we would expect the speaker optimality parameter to always be positive. The prior for the intercept should therefore not include neagtive values. However, in the context of our study, the youngest children performed below chance and increased with age. This pattern can only be captured by the model if we allow the intercept to take negative values. To combine these considerations, the prior for the intercept is skewed towards being positive, but also includes negative values. The prior on the slope is agnostic about the shape of the developmental trajectory.
childModelParam <- '
...
var model  = function(){
  ...
  var so_slope = uniformDrift({a: -5, b: 5, width: 0.2})
  var so_int = uniformDrift({a: -3, b: 7, width: 0.2})
