...
# model predictions
child_ex3_pragm_model_predictions_developmental_noise <- readRDS("../../stats/saves/child_ex3_pragm_model_noise.rds") %>%
...
ggplot(developmental_noise_parameter, aes(x = age, y= y))+
  geom_line(size = 2)+
...
ggplot(child_model_predictions)+
  ...
  theme_few()+
  scale_color_manual(name="Model",
                     breaks=c("pragmatic","pragmatic_noise","flat_prior","prior_only"),
                     values= c("#859900","#00afaf","#6c71c4","#b58900"))+
...
child_cor_plot <- child_cor_model %>%
  ungroup()%>%
  mutate(Data = rep(child_cor_data$Data,4),
         Data_ci_lower = rep(child_cor_data$Data_ci_lower,4),
         Data_ci_upper  =rep( child_cor_data$Data_ci_upper,4))
...
ggplot(child_ex3_model_comparison, aes(x = model, y = logP, col = model))+
  ...
  ylim(-475,-400)+
  scale_color_manual(breaks=c("pragmatic","pragmatic_noise","prior_only_noise","flat_prior_noise"),
                    values= c("#859900","#00afaf","#b58900","#6c71c4"))
