...
**issue: here we are passing `adult_ex1_data` when we want `adult_ex1_data %>% filter(condition == "test")`**
...
prior_pref_summary <- adult_ex2_preference_data %>%
  group_by(condition)
...
makePriorList = function(p){
  c(1 - p, 1 - p, p)
}

makeInversePriorList = function(p){
  c(p, p, 1 - p)
}

model_params_ex3$prior = list(
  makePriorList(prior_pref_same$prior), 
  makeInversePriorList(prior_pref_same$prior), 
  makePriorList(prior_pref_diff$prior), 
  makeInversePriorList(prior_pref_diff$prior), 
  makePriorList(prior_novel_same$prior), 
  makeInversePriorList(prior_novel_same$prior), 
  makePriorList(prior_novel_diff$prior), 
  makeInversePriorList(prior_novel_diff$prior)
)
```
