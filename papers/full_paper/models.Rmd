...
#   chains = 4,
#   cores = 4,
#   inference_opts = list(method = "MCMC", samples = 3000, burn = 1000, verbose = T)
# )%>%
...
#   inference_opts = list(method = "MCMC", samples = 3000, burn = 1000, verbose = T)
# )%>%
#   saveRDS("../../stats/saves/child_ex3_prior_only_model.rds")

child_ex3_prior_only_model <- readRDS("../../stats/saves/child_ex3_prior_only_model.rds")
...
child_ex3_prior_only_model_noise <- child_ex3_prior_only_model %>%
  filter(Parameter %in% c("noise_int","noise_slope"))  %>%
  group_by(Parameter)%>%
  summarise(mean = estimate_mode(value), 
            ci_lower = hdi_lower(value),
...
#   inference_opts = list(method = "MCMC", samples = 3000, burn = 1000, verbose = T)
# )%>%
#   saveRDS("../../stats/saves/child_ex3_flat_prior_model.rds")

child_ex3_flat_prior_model <- readRDS("../../stats/saves/child_ex3_flat_prior_model.rds")
...
child_ex3_flat_prior_model_noise <- child_ex3_flat_prior_model %>%
  filter(Parameter %in% c("noise_int","noise_slope"))  %>%
  group_by(Parameter)%>%
  summarise(mean = estimate_mode(value), 
            ci_lower = hdi_lower(value),
...
### Plot for noise parameter

```{r visualizing noise parameters}
plot_noise_param <- data_frame(
  ...
  model = c(
    ...
    rep("flat_prior",41)),
  int = c(
    rep(child_ex3_pragm_model_noise%>%filter(Parameter=="noise_int")%>%pull(mean),41),
    rep(child_ex3_prior_only_model_noise%>%filter(Parameter=="noise_int")%>%pull(mean),41),
    rep(child_ex3_flat_prior_model_noise%>%filter(Parameter=="noise_int")%>%pull(mean),41)
    ),
  slope = c(
    rep(child_ex3_pragm_model_noise%>%filter(Parameter=="noise_slope")%>%pull(mean),41),
    rep(child_ex3_prior_only_model_noise%>%filter(Parameter=="noise_slope")%>%pull(mean),41),
    rep(child_ex3_flat_prior_model_noise%>%filter(Parameter=="noise_slope")%>%pull(mean),41)
    )
) %>%
  mutate(y = plogis(int + slope *age))

plot_child_noise_param_dev <- ggplot(plot_noise_param, aes(x = age, y= y, col = model))+
  geom_line(size = 2)+
  ylim(0,1)+
  theme_few()+
...
child_noise_param_by_model <- bind_rows(
  child_ex3_pragm_model%>%mutate(model="pragmatic"),
  child_ex3_prior_only_model%>%mutate(model="prior_only"),
  child_ex3_flat_prior_model%>%mutate(model="flat_prior")
) %>%
  filter(Parameter %in% c("noise_int", "noise_slope"))

plot_child_noise_param_by_model <- ggplot(child_noise_param_by_model, aes(x = value, col = model))+
  geom_density()+
...
ggarrange(plot_child_noise_param_by_model, plot_child_noise_param_dev, common.legend = T)
```
