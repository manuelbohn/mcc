...
bda <- webppl(
  ...
  data_var = "dataFromR",
  model_var = "model",
  inference_opts = list(method = "MCMC", samples = 5000, burn = 2500, verbose = T)
)

## check one iteration
filter(bda, Iteration == 1)

## check what things look like in JSON format
jsonlite::toJSON(data, pretty = T)

jsonlite::toJSON(allPriorProbs, pretty = T)

## distribution of noise
posterior.noise <- bda %>%
  filter(Parameter == "noise")
...
noisy.model <- posterior.predictive %>%
  mutate(Model="Noisy Model")%>%
  group_by(Model,Experiment,Change,Alignment) %>%
...
p.com <- bind_rows(noisy.model,pd)
...
plot3 <- data.frame(noisy.model$Experiment,pd$mean[pd$Model=="Data"],noisy.model$mean,rep("Data vs Noisy Model",8))
names(plot3) = c("Experiment","Data","Model","Comp")

plot_noise <- rbind(plot,plot3)

ggplot(data = plot_noise,aes(x = Model, y = Data, col = Comp)) +
  geom_point()+
  facet_grid(~Comp)+
  stat_cor(method = "pearson", label.x = 0.4, label.y = 0.9)+
  geom_smooth(method = "lm", se = F, col = "black")+
