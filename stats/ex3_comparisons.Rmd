...
allPriorProbs2 <- allPriorProbs

allPriorProbs2$priorProbs = list(
    c(0.0333333, 0.9666667), 
    c(0.9666667, 0.0333333),
    c(0.3583333, 0.6416667),
    c(0.6416667, 0.3583333),
    c(0.1666667, 0.8333333),
    c(0.8333333, 0.1666667),
    c(0.4083333, 0.5916667),
    c(0.5916667, 0.4083333))
...
prior.model <- posterior.predictive1 %>%
...
```{r model prior2 loglikelihood}
prior.loglikelihood2 <-'
var allConditions = dataFromR.allConditions
var allData = dataFromR.allData
var allPriorProbs = dataFromR.allPriorProbs
...
var model = function(){
  ...
  var conditionSpecificData =  _.filter(allData, 
   ...
   var conditionSpecificPriors =  _.filter(allPriorProbs, 
    ...
    var priorOnlyPredictions = Infer({method: "enumerate", model: function(){
      var obj = sample( Categorical({vs: two_objects, ps: conditionSpecificPriors[0].priorProbs}));
      return obj.shape == "circle" ? 1 : 0
    }})
...
llh.prior2 <- webppl(
  program_code = paste(rsaUtils, rsaModel2, prior.loglikelihood2, sep='\n'),
  data = list(allData = data, allConditions = allConditions, allPriorProbs = allPriorProbs2), 
  data_var = "dataFromR",
  model_var = "model",
  inference_opts = list(method = "forward", samples = 5000),
  chains = 3, cores = 3
)%>%
  mutate(Model = "Prior only - non normalized")
...
llh.prior.noise2 <- webppl(
  program_code = paste(rsaUtils, rsaModel2, prior.noise.loglikelihood2, sep='\n'),
  data = list(allData = data, allConditions = allConditions, allPriorProbs = allPriorProbs2), 
  data_var = "dataFromR",
  model_var = "model",
  inference_opts = list(method = "forward", samples = 5000),
  ...
  mutate(Model = "Prior only Noisy - non normalized")
...
likelihood_overview <- bind_rows(
  ...
  llh.prior.noise2,
  llh.prior.regPrior,
  llh.rsa.regPrior %>% filter(value != "-Infinity") %>% mutate(value = as.numeric(value)),
