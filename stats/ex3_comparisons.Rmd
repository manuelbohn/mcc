...
```{r rsa model2}
rsaModel2 <- 'var ObjectPrior = Categorical({vs: all_objects, ps: [0.1666667, 0.1666667, 0.8333333] })
...
var model = function(){
   ...
   var modelPredictions = pragmaticListener({label: "dax", point: 2 })
   var noisyModelPredictions = addNoise(modelPredictions, noise)
   map(data, function(d){ 
      observe(noisyModelPredictions, d)
    } )
  return [noise, modelPredictions]
}
...
obs_data <- data %>% filter (Change == "Same speaker", Alignment == "Congruent", Experiment =="Novelty") %>% select(correct_inf)
