...
observe(S1, utterance)
return {lexicon: lexiconName, obj: obj.shape == "circle" ? 1 : 0}
}})
}
...
var model = function(){
   var noise = uniform(0, 1)

  var conditionOutput = map(function(conditionInfo){

    var conditionSpecificData =  _.filter(allData, {Change: conditionInfo.Change, Alignment: conditionInfo.Alignment, Experiment: conditionInfo.Experiment})

   var conditionSpecificPriors =  _.filter(allPriorProbs, {Change: conditionInfo.Change, Alignment: conditionInfo.Alignment, Experiment: conditionInfo.Experiment})

   var modelPredictions = pragmaticListener({label: "dax", point: 2 }, conditionSpecificPriors.priorProbs)
  ...
  return {noisyModelPred: Math.exp(noisyModelPredictions.score(1)), noise: noise, Experiment: conditionInfo.Experiment, Change: conditionInfo.Change, Alignment: conditionInfo.Alignment}
...
Infer({model: model, method: "MCMC", samples: 10})
'
```
...
bda <- webppl(
  program_code = paste(rsaUtils, rsaModel2, sep='\n'),
...
jsonlite::toJSON(priorProbs, pretty = T)
