...
```{r model no prior noise loglikelihood}
no.prior.loglikelihood <-'
var allConditions = dataFromR.allConditions
var allData = dataFromR.allData
var allPriorProbs = dataFromR.allPriorProbs
...
llh.no.prior.noise <- webppl(
  program_code = paste(rsaUtils, rsaModel2, no.prior.noise.loglikelihood, sep='\n'),
  data = list(allData = data, allConditions = allConditions, allPriorProbs = allPriorProbs), 
  data_var = "dataFromR",
  model_var = "model",
  inference_opts = list(method = "forward", samples = 10000),
...
llh.no.prior <- webppl(
  program_code = paste(rsaUtils, rsaModel2, no.prior.loglikelihood, sep='\n'),
  data = list(allData = data, allConditions = allConditions, allPriorProbs = allPriorProbs), 
  data_var = "dataFromR",
  model_var = "model",
  inference_opts = list(method = "forward", samples = 10000),
...
likelihood_overview <- bind_rows(
  llh.no.prior.noise %>% mutate(Model = "No Prior"),
  llh.rsa.noise %>% mutate(Model = "Pragmatic"),
  llh.prior.noise %>% mutate(Model = "Prior only"))
...
bf <- data.frame(
  ...
  BayesFactor = c(
    exp(llh.plot$logP[llh.plot$Model== "Pragmatic"] - llh.plot$logP[llh.plot$Model== "Prior only"]),
    exp(llh.plot$logP[llh.plot$Model== "Pragmatic"] - llh.plot$logP[llh.plot$Model== "No Prior"]),
    exp(llh.plot$logP[llh.plot$Model== "Prior only"] - llh.plot$logP[llh.plot$Model== "No Prior"]))
)
...
saveRDS(bf, "../saves/ex3_bf.rds")

saveRDS(vp.bf, "../saves/ex4_bf.rds")
