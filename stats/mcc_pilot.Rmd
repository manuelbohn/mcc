...
First model below converges, however, the random effects for choiceAgent are highly correlated suggesting that there is not enough data to estimate them independently (makes sense in light of plot/table above). Therefore I would remove the random slopes within choiceAgent (AIC looks better when they are removed). 
What would you do with the non-significant interaction? I put it in there just to check. I did not hypothesize that there would be an interaction, therefore I would remove it. Model 2 reflects what I would suggest.
...
```{r}
#  final model
#  removing random effects for trial as there seems to be no evidence for a trial effect
lm2 <- glmer(correct ~ change + condition +
              (condition + change |id) + (1|choiceAgent), 
              data = data_cg, family = binomial, 
              control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
