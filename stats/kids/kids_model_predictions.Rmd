...
plot_inf <- ggplot(data = pre_so, aes(x = age_num, y = correct)) +
  ...
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), se = T, col = "black",fill = "darkgreen", alpha = .4, size = .4)+
...
var model  = function(){
  var so_slope = uniformDrift({a: -3, b: 3, width: 0.4})
  var so_slope_quad = uniformDrift({a: -3, b: 3, width: 0.4})
  var so_int = uniformDrift({a: -3, b: 3, width: 0.4})

  foreach(function(row){
    var age = row.age_num
    var speakerOptimality = so_int  + so_slope * (age - infData[0].minage) + so_slope_quad * Math.pow((age - infData[0].minage),2)
  ...
  return _.flatten(pref_params).concat(
    [
      ...
      {condition: "NA", param: "so_slope_quad", val: so_slope_quad},
...
so_plot <- child_ex1_data %>%
  group_by(id,age_num,minage)%>%
  summarise(correct = mean(correct))%>%
  mutate(model = model_params$NA_so_int + model_params$NA_so_slope * (age_num - minage) + model_params$NA_so_slope_quad * (age_num - minage)^2)

ggplot(data = so_plot, aes(x = age_num, y = model)) +
...
lin_params_by_age<- webppl(
  program_code = paste(rsaUtils, rsaModel, linParamModelWebppl , sep='\n'),
  data = pre_data,
  data_var = "dataFromR",
  model_var = "model",
  chains = 4,
...
lin_model_params <- lin_params_by_age%>%
  select(value) %>%
  map_df(bind_rows)%>%
  group_by(condition,param)%>%
  summarise(param_map = estimate_mode(val))%>%
...
lin_so_plot <- child_ex1_data %>%
  group_by(id,age_num,minage)%>%
  summarise(correct = mean(correct))%>%
  mutate(model = lin_model_params$NA_so_int + lin_model_params$NA_so_slope * (age_num - minage))

ggplot(data = lin_so_plot, aes(x = age_num, y = model)) +
  geom_line(alpha = .5, size = .7)+
  labs(x="Age",y="Speaker optimality parameter")+
  theme_few() +
  geom_hline(yintercept = 0, lty=2)+
...
lin_model_pred<- webppl(
  program_code = paste(rsaUtils, rsaModel, linModelPredWebppl , sep='\n'),
  data = list(ex3data = ex3_data_param, modelParams = lin_model_params),
  data_var = "dataFromR"
)

#saveRDS(model_pred, file = "../saves/kids_model_predictions.rds")

lin_pred <- lin_model_pred %>%
  gather(condition, mean, -age_bin)%>%
  separate(condition, into = c("model", "Speaker", "Alignment"), sep="_")%>%
  mutate(Speaker = ifelse(Speaker == "diff", "different_speaker","same_speaker"),
         Alignment = ifelse(Alignment == "cong", "congruent","incongruent"),
         model = "pragmatic_linear", 
         age = age_bin+3)%>%
  select(-age_bin)
```

```{r}
linFlatModelPredWebppl <- '
...
var predict_by_age = function(age_bin){
    ...
    var diff_priorProbs_congruent = [0.5, 0.5, 0.5]

    var diff_priorProbs_incongruent = [0.5, 0.5, 0.5]
    ...
    var same_priorProbs_congruent = [0.5, 0.5, 0.5]

    var same_priorProbs_incongruent = [0.5, 0.5, 0.5]
...
model_pred_flat_lin <- webppl(
  program_code = paste(rsaUtils, rsaModel, linFlatModelPredWebppl , sep='\n'),
...
var predict_by_age = function(age_bin){
    ...
    var rsaPredictions_diff_cong =  Infer({method: "enumerate", model: function(){
      var obj = sample( Categorical({vs: all_objects, ps: diff_priorProbs_congruent}));
      return obj.shape == "circle" ? 1 : 0
       }})

    var rsaPredictions_diff_incong =Infer({method: "enumerate", model: function(){
      var obj = sample( Categorical({vs: all_objects, ps: diff_priorProbs_incongruent}));
      return obj.shape == "circle" ? 1 : 0
       }})
    ...
    var rsaPredictions_same_cong = Infer({method: "enumerate", model: function(){
      var obj = sample( Categorical({vs: all_objects, ps: diff_priorProbs_incongruent}));
      return obj.shape == "circle" ? 1 : 0
       }})

    var rsaPredictions_same_incong = Infer({method: "enumerate", model: function(){
      var obj = sample( Categorical({vs: all_objects, ps: same_priorProbs_incongruent}));
      return obj.shape == "circle" ? 1 : 0
       }})

   var predictions_diff_cong  =   Math.exp(rsaPredictions_diff_cong.score(1))
   var predictions_diff_incong  =   Math.exp(rsaPredictions_diff_incong.score(1))
   var predictions_same_cong  =   Math.exp(rsaPredictions_same_cong.score(1))
   var predictions_same_incong  =   Math.exp(rsaPredictions_same_incong.score(1))



  return extend(
      {predictions_diff_cong,
      predictions_diff_incong,
      predictions_same_cong,
      predictions_same_incong,
      age_bin
    })
...
lin_model_pred_prior<- webppl(
  program_code = paste(rsaUtils, rsaModel, linPriorModelPredWebppl , sep='\n'),
  data = list(ex3data = ex3_data_param, modelParams = lin_model_params),
  data_var = "dataFromR"
)
...
lin_pred_prior <- lin_model_pred_prior %>%
  gather(condition, mean, -age_bin)%>%
  separate(condition, into = c("model", "Speaker", "Alignment"), sep="_")%>%
  mutate(Speaker = ifelse(Speaker == "diff", "different_speaker","same_speaker"),
         Alignment = ifelse(Alignment == "cong", "congruent","incongruent"),
...
model_params <- readRDS("../../stats/saves/child_model_param.rds")%>%
  select(value) %>%
  map_df(bind_rows)%>%
...
ex3_data_param <- ex3_data%>%
  mutate(different_speaker_pref_int = model_params$pref_int_different_speaker,
         different_speaker_pref_slope = model_params$pref_slope_different_speaker,
         so_int = model_params$so_int,
         so_slope = model_params$so_slope,
         same_speaker_pref_int = model_params$pref_int_same_speaker,
         same_speaker_pref_slope = model_params$pref_slope_same_speaker,
...
var model = function(){
  ...
  var output = map(function(row){
    ...
    return  [row.age_num + "/" + row.Speaker + "/" + row.Alignment + "/" + row.correct_inf, Math.exp(noisyModelPredictions.score(1))]
...
noisy_pred <- noisy_model_pred %>%
  filter(!(Parameter %in% c("noise"))) %>%
  separate(Parameter, into = c("age", "speaker", "alignment", "correct"), sep="/")%>%
  mutate( model = "noise parameter across age")%>%
  group_by(model, age,alignment,speaker, correct) %>%
...
log_like_noisy_model <- noisy_pred %>%
  mutate(like = ifelse(correct == "1",mean,1-mean),
         log_like = log(like))%>%
  group_by(model)%>%
...
x <- data_frame(
  age = rep(seq(0,2, by = 0.05),8),
  param = c(rep("-2/-2",41),rep("-2/-1",41),rep("-1/-1",41),rep("-1/0",41),rep("0/-1",41),rep("1/1",41),rep("2/1",41),rep("2/2",41)),
  int = c(rep(-2,41),rep(-2,41),rep(-1,41),rep(-1,41),rep(0,41),rep(1,41),rep(2,41),rep(2,41)),
  slope = c(rep(-2,41),rep(-1,41),rep(-1,41),rep(0,41),rep(-1,41),rep(1,41),rep(1,41),rep(2,41))
) %>%
  mutate(y = plogis(int + slope *age))

ggplot(x, aes(x = age, y= y,col = param))+
  geom_line()+
  ylim(0,1)


plot.posterior.noise <- posterior.noise%>%
  group_by(Parameter)%>%
  summarise(mean = estimate_mode(value))%>%
...
x <- data_frame(
  age = rep(seq(0,2, by = 0.05),1),
  `int/slope` = c(rep(paste(round(plot.posterior.noise$noise_int,2),round(plot.posterior.noise$noise_slope,2), sep = "/"),41)),
  int = c(rep(plot.posterior.noise$noise_int,41)),
  slope = c(rep(plot.posterior.noise$noise_slope,41))
) %>%
  mutate(y = plogis(int + slope *age))

ggplot(x, aes(x = age, y= y,col = `int/slope`))+
  geom_line()+
  ylim(0,1)
...
child_ex3_pragm_model_predictions<- child_ex3_pragm_model_predictions%>%
  mutate(mean = value,
         ci_lower = value,
         ci_upper = value)
...
p_data <- child_ex3_data %>%
  mutate(age_bin = age_num - 3)%>%
  group_by(speaker, alignment ,age_bin, id)
...
ggplot(models)+
  ...
  geom_smooth(data = p_data, aes(x = age_num, y = correct_inf), col = "black", method = "loess", span = 2, se = T,  alpha = .5, size = .4)+
  #geom_hline(yintercept = 0.5, lty=2)+
  labs(y="proportion more informative")+
  ylim(-0.05,1.05)+
  facet_grid(speaker~alignment)+
...
cor_plot <- model %>%
  ungroup()%>%
  mutate(Data = rep(data$Data,4),
         Data_ci_lower = rep(data$Data_ci_lower,4),
         Data_ci_upper  =rep( data$Data_ci_upper,4))
...
var model = function(){
  ...
  var output = map(function(row){
    ...
    display(JSON.stringify(row.correct_inf))
...
noisy_model_pred<- webppl(
  program_code = paste(rsaUtils, rsaModel, noisyModelPredWebpplLoglike , sep='\n'),
  data =list(ex3data = ex3_data_param),
  data_var = "dataFromR",
  model_var = "model",
  chains = 4,
...
#   data = list(Data = adult_ex4_data, allConditions = allConditionsEx4, predPrior = predPriorEx4, noise = ex3_pragm_model_noise$mean),
#   data_var = "dataFromR",
#   model_var = "model",
#   chains = 4, 
#   cores = 4,
