...
model_pred_across <- readRDS("../saves/kids_model_predictions.rds")
...
kids_model_pred <- model_pred %>%
  mutate(age_bin = age_bin.age_bin)%>%
  select(rsaPredictions_diff_cong,rsaPredictions_diff_incong,rsaPredictions_same_cong,rsaPredictions_same_incong,age_bin)%>%
  gather(condition, prop_informative, -age_bin)%>%
  separate(condition, into = c("model", "Speaker", "Alignment"), sep="_")%>%
  mutate(Speaker = ifelse(Speaker == "diff", "different_speaker","same_speaker"),
         Alignment = ifelse(Alignment == "cong", "congruent","incongruent"))
```

### Noisy RSA predictions by age bin
...
ageParams <- data.frame(
  age_bin = rep(seq.int(0, 1.95, 0.05),4),
  Speaker = c(rep("same_speaker",80),rep("different_speaker",80)),
  Alignment = c(rep("congruent",40),rep("incongruent",40),rep("congruent",40),rep("incongruent",40)),
  SpeakOpt = rep(model_params_by_age$so_map,4))

listPrior <- data.frame(
  a = 1:length(ageParams$age_bin),
  ob1 = c(1- model_params_by_age$same_priorReg_map,
          model_params_by_age$same_priorReg_map,
          1- model_params_by_age$diff_priorReg_map,
          model_params_by_age$diff_priorReg_map),
  ob2 = c(1- model_params_by_age$same_priorReg_map,
          model_params_by_age$same_priorReg_map,
          1- model_params_by_age$diff_priorReg_map,
          model_params_by_age$diff_priorReg_map),
  ob3 = c(model_params_by_age$same_priorReg_map,
          1- model_params_by_age$same_priorReg_map,
          model_params_by_age$diff_priorReg_map,
          1- model_params_by_age$diff_priorReg_map))%>%
  group_by(a)%>%
  mutate(list = list(c(ob1,ob2,ob3)))%>%
  pull(list)


ageParams$priorP <- listPrior
...
var allConditions = dataFromR.ageConditions
var allData = dataFromR.ex3data
var allParams = dataFromR.ageParams
...
var model = function(){
  ...
  var conditionSpecificData =  _.filter(allData, {binned_age: conditionInfo.age_bin, Speaker: conditionInfo.Speaker, Alignment: conditionInfo.Alignment})

      var ageConditionSpecificParams =  _.filter(allParams, {age_bin: conditionInfo.age_bin, Speaker: conditionInfo.Speaker, Alignment: conditionInfo.Alignment})

    //display(JSON.stringify(ageConditionSpecificParams[0].priorProbs))

   var modelPredictions = pragmaticListener({label: "dax", point: 2 }, ageConditionSpecificParams[0].priorProbs,   ageConditionSpecificParams[0].SpeakOpt)
  ...
  return  [conditionInfo.age_bin + "-" + conditionInfo.Speaker + "-" +  conditionInfo.Alignment, Math.exp(noisyModelPredictions.score(1))]
...
var ageParams = dataFromR.ageParams

display(JSON.stringify(ageParams))
...
var model = function(){
 ...
 // var predict_by_age = function(age_bin){
   ...
   var ageConditionSpecificData =  _.filter(ex3Data, {binned_age: conditionInfo.age_bin, Speaker: conditionInfo.Speaker, Alignment: conditionInfo.Alignment})
    
   var ageConditionSpecificParams =  _.filter(ageParams, {binned_age: conditionInfo.age_bin, Speaker: conditionInfo.Speaker, Alignment: conditionInfo.Alignment})

 display(JSON.stringify(ageConditionSpecificParams))
      
    var rsaPredictions = pragmaticListener({label: "dax", point: 2 }, ageConditionSpecificParams[0].Prior,   ageConditionSpecificParams[0].SpeakOpt)

    var noisyRsaPredictions_diff_cong = addNoise(rsaPredictions_diff_cong, noise)
    map(function(d){ 
    observe(noisyRsaPredictions_diff_cong, d.correct_inf)
    }, ageConditionSpecificData)
  // display(conditionInfo.age_bin + "_" + conditionInfo.Speaker + "_" +  conditionInfo.Alignment)
  // display(Math.exp(noisyRsaPredictions_diff_cong.score(1)))
...
//  }
//  var predictions_by_age = map(predict_by_age, modelParams)
  //display(JSON.stringify(predictions_by_age))
//  return predictions_by_age
}
'
```
...
noisy_model_pred<- webppl(
  ...
  data =list(ex3data = ex3_data %>% mutate(binned_age = as.numeric(as.character(binned_age))), 
...
posterior.predictive <- noisy_model_pred %>%
  filter(!(Parameter %in% c("noise", "priorExponent"))) %>%
  separate(Parameter, into = c("age_bin", "Speaker", "Alignment"), sep="-")%>%
  group_by(age_bin,Alignment,Speaker) %>%
...
ggplot(kids_model_pred)+
  ...
  geom_line(aes(age_bin, prop_informative), col = "black", size = 0.4, alpha = 1) + 
  geom_hline(yintercept = 0.5, lty=2)+
...
ggplot(posterior.predictive)+
  #geom_jitter(data = p_data, aes(x = age_bin, y = correct_inf), width = .025,height = .025, alpha = .2)+
  geom_ribbon(aes(x =age_bin, ymin = ci_lower, ymax = ci_upper), alpha = .4) +
  geom_line(aes(age_bin, mean), size = 1, alpha = 1, linetype = 1) + 
