...
for (f in files) {
  ...
  raw_data_2 <- bind_rows(raw_data_2, id)
}
...
d_raw_2 <-raw_data_2 %>%
  ...
  select(subid,subage,trial,correct,test_date)
...
raw_data_trees <- data_frame()
for (f in files) {
  jf <- paste("~/Work/MCC/git-mcc/kids_info_pilot/trees/",f,sep="")
  jd <- jsonlite::fromJSON(paste(readLines(jf), collapse=""))
  date <- str_sub(jf,50,str_length(jf)-11)
  id <- as_data_frame(jd$data$data)%>% mutate(test_date = date)
  raw_data_trees <- bind_rows(raw_data_trees, id)
}

d_trees <- raw_data_trees %>%
  filter(trial != "train1",
         trial != "train2",
         trial != "filler1",
         trial != "filler2",
...
for (f in files) {
  ...
  jd <- jsonlite::fromJSON(paste(readLines(jf), collapse=""))
  date <- str_sub(jf,63,str_length(jf)-11)
  id <- as_data_frame(jd$data$data)%>% mutate(test_date = date)
  raw_data <- bind_rows(raw_data, id)
}
...
for (f in files) {
  ...
  jd <- jsonlite::fromJSON(paste(readLines(jf), collapse=""))
  date <- str_sub(jf,61,str_length(jf)-11)
  id <- as_data_frame(jd$data$data)%>% mutate(test_date = date)
  raw_data <- bind_rows(raw_data, id)
}
...
log <- read_excel("../../../MCC-subject_log.xlsx", 1)%>%
  filter(Condition == "inf" | Condition == "inf2")%>%
  select(subid,experimenter,keep_drop,sex,dob, English, parent1_ed,parent2_ed)%>%
  mutate(language_status = ifelse(English == "100" | English == "90-95", "mono_ling", "bi_ling"))

d <- left_join(d_sum,log, by = "subid")%>%
  ...
  filter(language_status != "NA", 
         English != "NA")


write_csv(d, "../data/kids_inf_data_megan.csv")
...
ggplot() +
  geom_jitter(data = p1, aes(x = sex, y = correct, col = subage, alpha = .2),width = .3,height = .025)+
  geom_pointrange(data = p2, aes(x = sex, y = mean, col = subage, ymin = ci_lower, ymax = ci_upper),size = .8)+
...
p5 <- d %>%
  group_by(language_status,age_num, subid) %>%
  summarise(correct = mean(correct)) 

ggplot(data = p5, aes(x = age_num, y = correct,col = language_status)) +
